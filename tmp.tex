...
We use a SigLIP diagonal loss: each image is paired only with its own synthesised
caption, and all other image--text combinations are treated as negatives:
\begin{equation}
    \mathcal{L}_{\mathrm{it}}
    = -\frac{1}{B}\sum_{i=1}^{B}\sum_{j=1}^{B}
      \log\sigma\!\bigl(y^{\mathrm{it}}_{ij}\cdot\tau\langle\mathbf{z}_i^v,\mathbf{z}_j^t\rangle\bigr),
    \quad y^{\mathrm{it}}_{ij} = \begin{cases}+1 & i=j \\ -1 & i\neq j.\end{cases}
\end{equation}
